...
First we need to specify what our model is in a helper function like so:
...
fit.season.model = function( dat, outcomename, lagless = FALSE ) {

    if ( lagless ) {
        # fit model without any lags.  This is used for smoothing the series.
        form = paste( outcomename, " ~ month + temperature + Q2 + Q3 + Q4" )
    } else {
        form = paste( outcomename, " ~ lag.outcome + month + temperature + Q2 + Q3 + Q4 + lag.temperature + lag.Q2 + lag.Q3 + lag.Q4" )
    }
...
A seasonality model on NJ data
```{r}
data( newjersey )
head( newjersey )
envelope = process.outcome.model( "compwarr.cs", newjersey, t0=-7, R = 100, 
