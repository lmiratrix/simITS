...
test_that("The four smoothing calls work", {
  ...
  dat1 = add_lagged_covariates( dat=dat, outcomename="Y",
                                covariates = c("Q2","Q3","Q4") )
  head( dat1 )

  dat2 = add_lagged_covariates( dat=dat, outcomename="Y", 
  ...
  M0 = fit.season.model( dat.pre, outcomename="Y", timename = "month" )
  expect_equal( length( coef( M0 ) ), 9 )

  M0 = fit.season.model( dat.pre, outcomename="Y", timename = "month", lagless = TRUE )
...
test_that( "Making formula with and without lags works", {
  ...
  fm2 = make_fit_season_model( ~ A + B, ~ B )
  attr( fm, "formula" )
  attr( fm, "lag.formula" )
  expect_equal( attr( fm, "formula" ), attr( fm2, "formula" ) )
